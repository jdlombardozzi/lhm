  ...
  module Chunker
    class Range
      ...
      def initialize(migration, connection = nil, origin_key_columns = [], options = {})
        ...
        @id_column = origin_key_columns.first
      ...
      def execute
        ...
        while @next_to_insert <= @limit || (@start == @limit)
          ...
          begin
            affected_rows = ChunkInsert.new(@migration, @connection, bottom, top, @id_column, @retry_options).insert_and_return_count_of_rows_created
      ...
      def upper_id(next_id, stride)
        sql = "SELECT #{@id_column} FROM `#{ @migration.origin_name }` WHERE #{@id_column} >= #{ next_id } ORDER BY #{@id_column} LIMIT 1 OFFSET #{ stride - 1}"
